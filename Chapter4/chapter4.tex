...
In this section we present qualitative and quantitative results on two datasets, Scene Flow \citep{MIFDB16} and KITTI \citep{Geiger2012CVPR,Menze2015CVPR}.
%Firstly, in Section ?? we experiment with different variants of our model and justify a number of our design choices using the Scene Flow dataset \citep{MIFDB16}.
...
In \tbl{kitti_test} we evaluate the performance of our model on the KITTI 2012 and 2015 stereo datasets \citep{Geiger2012CVPR,Menze2015CVPR}. These consist of challenging and varied road scene imagery collected from a test vehicle. Ground truth depth maps for training and evaluation are obtained from LIDAR data. KITTI is a prominent dataset for benchmarking stereo algorithms. The downside is that it only contains $200$ training images, which handicaps learning algorithms. for this reason, we pre-train our model on the large synthetic dataset, Scene Flow \citep{MIFDB16}. This helps to prevent our model from over-fitting the very small KITTI training dataset. We hold out 40 image pairs as our validation set. In \tbl{kitti2012_2} and \ref{tbl:kitti2015_2} we compare our method to other approaches on the KITTI 2012 and 2015 datasets.%\footnote{Full leaderboard:~\url{www.cvlibs.net/datasets/kitti/}}. 
%, by a notable margin. We improve on state-of-the-art by XX\% and XX\% for KITTI 2015 and 2012 respectively.
...
MC-CNN \citep{zbontar2016stereo} 			& 3.90 & 5.45 & 2.43 & 3.63 & 1.64 & 2.39 & 0.7 px & 0.9 px & 67 \\
%PBCP \citep{Seki2016BMVC} 					& 3.62 & 5.01 & 2.36 & 3.45 & 1.62 & 2.32 & 0.7 px & 0.9 px & 68 \\
Displets v2 \citep{guney2015displets} 		& 3.43 & 4.46 & 2.37 & 3.09 & 1.72 & 2.17 & 0.7 px & 0.8 px & 265 \\
GC-Net \citep{kendall2017end}				& 2.71 & 3.46 & 1.77 & 2.30 & 1.12 & 1.46 & \textbf{0.6 px} & \textbf{0.7 px} & 0.9 \\ \hline
Probabilistic GC-Net (this work)           	& \textbf{2.65} & \textbf{3.40} & \textbf{1.72} & \textbf{2.21} & \textbf{1.08} & \textbf{1.39} & \textbf{0.6 px} & \textbf{0.7 px} & 0.9 \\
\end{tabular}}
	\caption{\textbf{KITTI 2012 test set results} \citep{Geiger2012CVPR}. This benchmark contains 194 train and 195 test gray-scale image pairs.}
\vspace{5 mm}
	\label{tbl:kitti2012_2}
\end{subtable}
\vspace{7 mm}
\begin{subtable}[t]{\linewidth}
\centering
